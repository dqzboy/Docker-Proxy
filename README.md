...
## 📝 准备工作
⚠️  **重要**：选择一台国外服务器(1C1G的即可，对于磁盘大小主要取决于你拉取镜像频率与保存缓存时长决定)，并且未被墙。对于域名，无需进行国内备案。你也可以通过[CLouDNS](https://www.cloudns.net/)申请免费域名。在一键部署过程中，如果选择安装Caddy，它将自动配置HTTPS。若选择部署Nginx服务，则需要自行申请一个免费的SSL证书，或者通过其他方式来实现SSL加密。
